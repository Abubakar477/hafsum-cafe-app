...
export default function LoginScreen({
  ...
  const tabAnim = useRef(
    new Animated.Value(initialTab === 'login' ? 0 : 1)
  ).current;
  ...
  const getFirebaseErrorMessage = (error) => {
    ...
    switch (code) {
      ...
      case 'auth/admin-restricted-operation':
        return 'Guest sign-in is not enabled in Firebase Authentication.';
  ...
  const handleGuest = async () => {
    ...
    try {
      console.log('Starting Firebase anonymous guest login...');

      // IMPORTANT:
      // This calls Firebase Anonymous Authentication.
      const result = await signInAsGuest();

      console.log('Firebase guest login successful:', result);
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {guestLoading ? (
                <ActivityIndicator color="#492760" />
              ) : (
                <>
                  <Ionicons
                    name="walk-outline"
